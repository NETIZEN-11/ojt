...
from app.domain.enums import SeverityLevel, ReviewStatus, ReviewLabel
...
class BaselineRepository(BaseRepository[Baseline]):
    ...
    async def get_active_for_suite(self, suite_id: UUID) -> Optional[Baseline]:
        result = await self.session.execute(
            select(Baseline).where(Baseline.suite_id == suite_id, Baseline.is_active == True)
        )
        return result.scalar_one_or_none()

    async def get_by_suite_version(self, suite_id: UUID, suite_version: int) -> Optional[Baseline]:
        result = await self.session.execute(
...
class BaselineItemRepository(BaseRepository[BaselineItem]):
    ...
    async def get_by_baseline_and_test_case(self, baseline_id: UUID, test_case_id: UUID) -> Optional[BaselineItem]:
        result = await self.session.execute(
...
class RegressionRepository(BaseRepository[Regression]):
    ...
    async def get_by_run_and_test_case(self, run_id: UUID, test_case_id: UUID) -> Optional[Regression]:
        result = await self.session.execute(
...
class SeverityFindingRepository(BaseRepository[SeverityFinding]):
    ...
    async def get_by_regression(self, regression_id: UUID) -> Optional[SeverityFinding]:
        result = await self.session.execute(
...
class ReviewQueueRepository(BaseRepository[ReviewQueue]):
    ...
    async def get_by_regression(self, regression_id: UUID) -> Optional[ReviewQueue]:
        result = await self.session.execute(
...
class ReviewLabelRepository(BaseRepository[ReviewLabelRecord]):
    ...
    async def get_latest_by_review(self, review_id: UUID) -> Optional[ReviewLabelRecord]:
        result = await self.session.execute(
