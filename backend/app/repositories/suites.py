...
from app.domain.enums import TestCaseCategory, ExpectedBehaviorType
...
class TestSuiteRepository(BaseRepository[TestSuite]):
    ...
    async def get_by_name(self, name: str) -> Optional[TestSuite]:
        result = await self.session.execute(select(TestSuite).where(TestSuite.name == name))
        return result.scalar_one_or_none()

    async def get_latest_version(self, suite_id: UUID) -> Optional[TestSuite]:
...
class TestSuiteVersionRepository(BaseRepository[TestSuiteVersion]):
    ...
    async def get_latest(self, suite_id: UUID) -> Optional[TestSuiteVersion]:
        result = await self.session.execute(
...
class TestCaseRepository(BaseRepository[TestCase]):
    ...
    async def get_by_suite_and_id(self, suite_id: UUID, test_case_id: str) -> Optional[TestCase]:
        result = await self.session.execute(
...
class TestCaseVersionRepository(BaseRepository[TestCaseVersion]):
    ...
    async def get_latest(self, test_case_id: UUID) -> Optional[TestCaseVersion]:
        result = await self.session.execute(
